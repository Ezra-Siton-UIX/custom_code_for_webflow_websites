export function fancybox_modals(lenis){
  ...
  project_card_fancyboxes.forEach(project_card_fancybox => {
    ...
    let logoHTML = '';
    if (project_logo && project_logo.src && project_logo.src.trim() !== '' && project_logo.src !== placeholderSrc) {
      logoHTML = `<img style="
    width: 60px;
    height: 60px;
    padding: 2px;
    ...
    if (project_image) {
      ...
      project_card_fancybox.setAttribute('data-caption', `
  ...
  if(show_banner_always || banner_view == null && window.location.pathname == '/' && show_modal_cms_value){
